...
在现有 Python Agent Service POC 上增加 AI 员工、Workflow、Case、门店记忆、审批和 Java 业务系统适配能力。系统复用当前 `RunnerService`、自研 `ResponsesRuntime`、`ToolGateway` 和 `ModelProviderRegistry`，不推倒重写已经验证的 Tool Loop、权限、审计和证据检查。Runtime 通过 `AgentRuntime` 公共契约隔离，具体决策见 [ADR-001](技术-07-ADR-001-Agent-Runtime路线.md)。
...
flowchart TB
    ...
    WF --> RT[AgentRuntime]
    RT --> RESP[ResponsesRuntimeAdapter]
    RESP --> REG[Model Provider Registry]
    REG --> BAILIAN[百炼 Responses API]
    RESP --> GW[Tool Gateway]
...
| 组件 | 权威职责 |
|---|---|
| Agent Service | AI 员工、Workflow、Run、Case、门店记忆/画像、审批、辅助判断、验证版本、审计和用量 |
...
- `AgentRuntime` 是业务层唯一依赖的运行契约；首期 `ResponsesRuntimeAdapter` 只管理一次 Run 内的模型请求、Tool Loop、证据检查和最终输出。
...
风险日读取和门店周报查询统一携带 `dataset_code`、`stat_date`、`data_as_of`、`refresh_batch_id`、`refresh_status` 和 `refresh_completed_at`。风险任务由系统每日 07:00 统一调度。

Agent Service 通过逻辑 `DataReadinessProvider` 查询“租户 + 数据集 + 统计日”的就绪状态；首期数据集编码为 `RISK_STORE_DAILY` 和 `STORE_MANAGER_WEEKLY`。刷新未完成、成功但无业务数据和补数必须区分，不能把结果表零行直接解释为没有风险。现有控制表、任务日志或新增最小分区状态记录中的实际来源在实施前核验，不在设计阶段虚构物理表。
...
Agent Service 是员工状态、Credential、Scope 和动作审批的事实源。Java 不复制这些主数据，也不反向调用 Agent 鉴权；Java 校验请求签名、授权断言、租户/对象归属、幂等和自身业务规则。

Scope 变化不修改历史快照，但恢复执行使用“实例 Scope 快照 ∩ 员工当前独立 Scope”；写动作执行前必须再次校验该交集。
...
- Case 默认每 6 小时只查询 Question、`QUESTION_ORDER` 和 Java 命令记录，不默认重新调用模型；只有明确需要重新分析时才创建新的 Run。
...
Webhook 是 Best Effort 通知，不是可靠事件总线。
...
Validation 模块按 `FR-VALIDATION-001..006` 保存不可变数据集、Baseline、候选版本和逐图结果。它与生产风险 Workflow 隔离，生产配置只能在用户明确选择后经 Java 受控接口同步。
...
Agent Service 以追加事件保存门店记忆，并生成分层投影和 Store Profile。AI 店长画像以 `primary_store_id` 为明细边界，排行样本范围仅用于确定性聚合；每次 Run 固化画像版本。详细持久化和访问规则见 [Agent Service 设计](技术-02-Agent-Service设计.md)。

## 14. 实施 Epic

- **Epic A：AI 风险督导主线**，包括风险 Trigger、Case、审批、`agentRisk` Question 和确定性跟进；
- **Epic B：图片辅助判断**，包括已有图片复跑、人工复核和唯一 `aiInspection` Question 引用；
- **Epic C：规则验证与 Period 配置快照**，包括数据集、Baseline、候选版本、门槛、受控同步和 Period 快照。Period 快照是 Epic C 的前置任务，独立于 Agent 主线开发；
- **Epic D：AI 店长与门店画像**，包括门店运营执行复盘、门店记忆、Store Profile、周报和隐私排行。Epic D 独立验收，不阻塞 Epic A 风险督导上线。

公共底座中的 Runtime Adapter 和 `DataReadinessProvider` 在对应 Epic 首次使用前完成。`agentRisk` 是 Epic A 的 Java 前置适配任务，不视为当前代码已有能力。
