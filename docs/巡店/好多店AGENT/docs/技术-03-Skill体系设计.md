...
Workflow 在调用 Runtime 前，必须按当前 `SkillSnapshot` 的 Input Schema 校验业务输入并生成不可变 `RuntimeInput`。身份、Scope、审批和租户字段仍由 `RunContext` 后端注入，不能混入业务输入或由模型覆盖。第一次执行前保存该输入快照及 `runtime_input_sha256`；同一 Step 的模型重试复用同一快照。业务输入需要重新查询或更新时，必须由 Workflow 显式生成新的 Step 或输入版本。
...
调度时间、阈值、审批和业务策略参数存放在 Workflow Settings，不通过复制租户私有 Skill 表达。

## 7. 当前与目标能力

当前 POC 已验证 `risk_store_analysis`，首期再发布 `store_manager_review`，覆盖门店运营执行复盘。两个 Skill 共用同一 Runtime、Tool Gateway、记忆快照和审计契约。
...
AI 运营经理、食安等后续岗位不属于首期实现，不在当前 Skill Catalog 发布。

## 8. `store_manager_review` Skill Contract

| 项目 | 设计 |
|---|---|
| 适用 Role | `AI_STORE_MANAGER` |
| 输入 | `primary_store_id`、当前门店事实、固定历史窗口、异常引用、周报周期、排行投影、`profile_snapshot_id` |
| 输出 | 当前门店画像、历史变化、异常事项、周报草稿、当前门店自己的排名、数据来源和限制 |
| 允许 Tool | `query_store_snapshot`、`query_store_history`、`query_store_memory`、`query_store_profile`、`query_store_rank_projection`、`generate_store_weekly_report_draft` |
| 禁止能力 | 读取其他门店详情、创建 Case/Question、发送消息、修改任务或业务状态、上传图片、自由 SQL |

门店画像只作为带来源的上下文输入。Skill 可以总结门店返回和最终复审的 `FALSE_POSITIVE` 记忆，但不能凭模型文本新增或改写记忆事件或标签；需要更正时必须由系统事件或人工复审流程产生新版本。输出只允许使用当前门店排行投影，不接触其他门店排名明细。
...
6. Run 保存模型、Skill/Prompt 版本与摘要、Tool、结构化结果和用量，不默认保存完整 Provider 请求/响应；
